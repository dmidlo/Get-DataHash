...
🛑 **They cannot be used in cryptographic workflows** →  But will be soon. I had to make a single compromise to prove out a couple other ideas and implement the LiteDB serializer.   Shower thoughts over the weekend made me realize that LiteDB and BSON as a serializer introduces a wrapper artifact to get the job done for non-dict like objects; scalars, etc...  this wrapper artifact could potentially represent a vector for reverse engineering if these digests are used for more than testing graph semantics and schema purposes.  The gen2 alogrithm has already solved this problem and LiteDB will be written out prior to a C# rewrite and will enable the objective of true integral identity verification of semi-resolved graphs of Powershell DTOs. That being said This version is indeed production ready understanding that limitation.  As is; this is still the best and only tool for its purpose available and is perfect addtion to your development workflows and in your pester tests.
